...
function get_price_ser($id){
    $html = '';
    require 'conn.php';
    $query = "SELECT p.*, s.ID ids, s.NAME_SERV, s.SPECID FROM `price` p, services s where p.id_service = s.ID and p.id_user = $id GROUP BY S.ID";
  //  echo $query;
    $result = mysqli_query($link, $query) or die("Ошибка " . mysqli_error($link)); 
    $rows = mysqli_fetch_all($result, MYSQLI_ASSOC);
  ...
  echo  '<div class="list-group-item py-1 pr-3">
        <div class="row">
